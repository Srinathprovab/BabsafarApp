...
class SearchFastTrackVC: BaseTableVC {
    ...
    static var newInstance: SearchFastTrackVC? {
        let storyboard = UIStoryboard(name: Storyboard.FastTrack.name,
                                      bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: self.className()) as? SearchFastTrackVC
        return vc
    }
